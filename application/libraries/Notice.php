...
class Notice
{
  ...
  function sum_user_disputes($user) {
      ...
      $open_disputes = $CI->disputes_model->sum_user_disputes($user);
  ...
  function user_view_balance($user) 
  {
    //print("ZZZ".$user);die;
    $CI =& get_instance();
      
    $CI->load->model('users_model');
    $user_data = $CI->load->users_model->get_username($user);
    
    //print_r($user_data['view_balance']);
    
    return $user_data['view_balance'];
  }
  ...
  function hold_balance($user, $currency, $type) 
  {
      ...
      if ($type == 1) {
        ...
        //print("<pre>");print_r($hold_balance);die;
  ...
  function check_amount_transaction($sum, $amount, $fee, $sender, $receiver, $type, $user, $id_transaction, $btc=0) {

...
class Exchange extends Private_Controller {
    ...
    $this->set_title(sprintf(lang('users menu exchange'), $this->settings->site_name));
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if ($currency == "debit_extra1") {
				...
				if($this->currencys->display->api_extra5 == "0") {
                  ...
                  //$rates = $this->fixer->get_rates($this->currencys->display->base_code, $this->currencys->display->extra5_code);
        			if ($this->cryptapi_model->get_current_rate()['status'] == 1) {
                      $rates = $this->cryptapi_model->get_current_rate()['btc_rate'];
                    } else {
                      $rates = $this->currencys->display->extra5_rate;
                    }
            ...
            //CHECK LIMITS
            //print($this->input->post("currency", TRUE)." ".$total);die;
            $limit_check = $this->users_model->check_user_limit($this->user['id'], $this->input->post("currency", TRUE), $total);
            ...
            if ($currency == "debit_extra5") {
              // new number format
              $rates_final = number_format($rates, 8, '.', '');
              $amount_final = number_format($amount, 2, '.', '');
              $total_final = number_format($total, 8, '.', '');
    ...
    $this->set_title(sprintf(lang('users menu exchange'), $this->settings->site_name));
		...
		if ($this->form_validation->run() == FALSE)
		{
            ...
            if ($main_currency == "debit_extra5") {
              // new number format
              $rates_final = number_format($rates, 8, '.', '');
              $amount_final = number_format($amount, 8, '.', '');
              $total_final = number_format($total, 2, '.', '');
              $total_fee_final = number_format($total_fee, 8, '.', '');
              $total_give_final = number_format($total_give, 8, '.', '');
            } else {
              // new number format
              $rates_final = number_format($rates, 2, '.', '');
              $amount_final = number_format($amount, 2, '.', '');
        ...
        //CHECK LIMITS
        //print($total_final);die;
        
        //pass TO cur_data
        $limit_check = $this->users_model->check_user_limit($this->user['id'], $currency, $total_final);

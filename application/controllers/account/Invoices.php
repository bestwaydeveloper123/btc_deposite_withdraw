...
class Invoices extends Private_Controller {
    ...
    {
      $date_xss = $this->security->xss_clean($this->input->get('datefrom'));
			$date_string = htmlentities($date_xss, ENT_QUOTES, "UTF-8");
			$filters['datefrom'] = $date_string;
    }
    if ($this->input->get('dateto'))
    {
      $date_xss = $this->security->xss_clean($this->input->get('dateto'));
			$date_string = htmlentities($date_xss, ENT_QUOTES, "UTF-8");
			$filters['dateto'] = $date_string;
    }
       ...
       $filter .= "&{$key}={$value}";
        ...
        if ($this->input->post())
        {
            if ($this->input->post('clear'))
            {
                ...
                {
                    $filter .= "&datefrom=" . $this->input->post('datefrom', TRUE);
                }
                if ($this->input->post('dateto'))
                {
                    $filter .= "&dateto=" . $this->input->post('dateto', TRUE);
                }
  ...
  function new_invoice()
  {
    ...
    $favlist = $this->users_model->get_user_favlist($this->user['id']);
    ...
    if ($this->form_validation->run() == FALSE)
		{
            ...
            if (strpos($this->input->post("receiver", TRUE), '@') !== false) {
              ...
              $this->session->set_flashdata('error', lang('users invoices error_3 invite'));
              redirect(site_url("account/invoices/new_invoice?invite&email=".$this->input->post("receiver", TRUE)));
      ...
      if (strpos($this->input->post("receiver", TRUE), '@') !== false) {
        ...
        $receiver = $this->users_model->get_usernamemail($this->input->post("receiver", TRUE));
      ...
      //add to FAV
            ...
            $receiver_id = $this->users_model->get_user_id($receiver);
            $this->users_model->add_fav($this->user['id'], $receiver_id);

	...
	{	
        ...
        $user = $this->users_model->get_user($this->user['id']);
          ...
          if ($this->form_validation->run() == FALSE)
          {
            ...
            //email check + invite
            if (strpos($this->input->post("receiver", TRUE), '@') !== false) {
              
              $this->session->set_flashdata('error', lang('users error form invite'));
              redirect(site_url("account/money_transfer?invite&email=".$this->input->post("receiver", TRUE)));
          ...
          else
          {
            ...
            //CHECK LIMITS
            //user id -> receiver
            $receiver_user_id = $this->users_model->get_user_id($receiver);
            $limit_check = $this->users_model->check_user_limit($receiver_user_id, $this->input->post("currency", TRUE), $this->input->post("amount", TRUE));
            
            //print($limit_check['status']);die;
            ...
            if ($limit_check['status'] == 0) {
              ...
              if ($currency = 'debit_base') { $cur_text = $this->currencys->display->base_code; }
              elseif ($currency = 'debit_extra1') { $cur_text = $this->currencys->display->extra1_code; }
              elseif ($currency = 'debit_extra2') { $cur_text = $this->currencys->display->extra2_code; }
              
              if ($limit_check['vrf_status'] == 0) {
                $msg = lang('users limits exceeded_receiver ini').' '.$limit_check['limit_sum_left'].' '.$cur_text.' '.lang('users limits exceeded_receiver ini2');
                
              } elseif ($limit_check['vrf_status'] == 1) {
                $msg = lang('users limits exceeded_receiver std').' '.$limit_check['limit_sum_left'].' '.$cur_text.' '.lang('users limits exceeded_receiver std2');
              } elseif ($limit_check['vrf_status'] == 2) {
                $msg = lang('users limits exceeded_receiver ext').' '.$limit_check['limit_sum_left'].' '.$cur_text.' '.lang('users limits exceeded_receiver ext2');
              }

               $user_receiver = $this->users_model->get_usernamemail($receiver);
               //print_r($user_receiver);die;
              
              $msg = str_replace("[USERNAME]",$user_receiver['first_name'].' '.$user_receiver['last_name'],$msg);
            ...
            if ($code_protect != NULL) {
              ...
              $status = "2";
					...
					if ($total_sender < $hold_balance) {
						...
						elseif ($user[$currency] > $sum) { //if we have enough money to send

							if ($protect == "none") {

                                //check $receiver username/email
                                 if (strpos($receiver, '@') !== false) {
                                    //echo 'true';
                                   $receiver = $this->users_model->get_usernamemail($receiver)['username'];
                                  }
                                //print_r($receiver['username']);die;
                                ...
                                /*
                                  //email notification for receiver
                                    ...
                                    $receiver_data = $this->users_model->get_user($receiver_user_id);
                                    $cur_index = '';
                                    ...
                                    $email_template = $this->template_model->get_email_template(35);

                                    $rawstring = $email_template['message'];

                                    // what will we replace
                                    $placeholders = array('[NAME]', '[SUM]', '[CUR]');
                                    $vals_1 = array($user_name, $amount, $cur_index);
                            ...
                            //LIMIT if receiver limited => status = 6
                            //if ($limit_check['status'] == 0) {
                              //$status = 6;
                            //}
 ...
 **************************************************************************************/
    ...
    {
        if (trim($username) != trim($current) && $this->users_model->user_exists($username))
        {

	...
	function confirm()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			if (!isset($this->input->post()['method'])) {
				$this->session->set_flashdata('error', lang('users withdrawal error_0 withdrawl'));
			} else {
            ...
            if ($method == "bitcoin") {
              ...
              $fee = number_format($this->input->post("fee", TRUE), 8, '.', '');
              $fee_fix = number_format($this->input->post("fee", TRUE), 6, '.', '');
            } else {
            ...
            if ($method == "Bitcoin") {
              ...
              $total_fee = number_format($total_fee_calc, 6, '.', '');
              $total_amount_calc = $amount + $total_fee;
              $total_amount = number_format($total_amount_calc, 6, '.', '');

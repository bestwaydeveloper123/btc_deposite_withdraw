...
class Settings extends Private_Controller {
    ...
    */
	function index()
	{
        ...
        $this->form_validation->set_rules('phone', lang('users input phone'), 'required|max_length[15]|min_length[8]|callback__check_phone');
        ...
        //notifications
        $notification_list = $this->settings_model->get_user_notifications($user);
    ...
    */
	function verification()
	{
		...
		//adding user country by ip
		if (!empty($_SERVER['HTTP_CLIENT_IP'])) {$ip = $_SERVER['HTTP_CLIENT_IP'];}
		elseif (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {$ip = (isset($_SERVER["HTTP_CF_CONNECTING_IP"])?$_SERVER["HTTP_CF_CONNECTING_IP"]:$_SERVER['REMOTE_ADDR']);}
		else {$ip = $_SERVER['REMOTE_ADDR'];}
		
		$user_country = $this->users_model->get_user_country($ip);

		//end get country
		//get phone code
		//$country_phonecode = "+7";
		
		$country_phonecode = $this->users_model->get_phone_prefix($user_country);
		...
		//end get phone code
         ...
         $this
			->add_js_theme('jquery-birthday-picker.js');
		 ...
		 $content_data['return']['zip'] = ""; $content_data['return']['city'] = ""; $content_data['return']['address'] = ""; $content_data['return']['phone'] = "";
		 if ($this->session->userdata['return_zip']) {$content_data['return']['zip'] = $this->session->userdata['return_zip']; }
		 if ($this->session->userdata['return_city']) {$content_data['return']['city'] = $this->session->userdata['return_city']; }
		 
         if ($this->session->userdata['return_address_1']) {$content_data['return']['address_1'] = $this->session->userdata['return_address_1']; }
         if ($this->session->userdata['return_address_2']) {$content_data['return']['address_2'] = $this->session->userdata['return_address_2']; }
		 
         if ($this->session->userdata['return_phone']) {$content_data['return']['phone'] = $this->session->userdata['return_phone']; }
         if ($this->session->userdata['return_company']) {$content_data['return']['company'] = $this->session->userdata['return_company']; }
         
         if ($this->session->userdata['return_b_day']) {$content_data['return']['b_day'] = $this->session->userdata['return_b_day']; }
         if ($this->session->userdata['return_b_month']) {$content_data['return']['b_month'] = $this->session->userdata['return_b_month']; }
         if ($this->session->userdata['return_b_year']) {$content_data['return']['b_year'] = $this->session->userdata['return_b_year']; }
		 
		 //if ($content_data['user']['zip'] == '') {$content_data['user'] = $content_data['return']['zip'];}
	...
	/**
	* Update standard verification settings
    */
	function standart_verification()
	{
		...
		$this->form_validation->set_rules('address_2', lang('users settings address_2'), 'max_length[300]|min_length[2]');
		$this->form_validation->set_rules('phone', lang('users input phone'), 'required|max_length[15]|min_length[8]|callback__check_phone');
        $this->form_validation->set_rules('bdayf_birthDay', lang('users input bday'), 'required');
		
		$post_data = $this->security->xss_clean($this->input->post());
		//
		/*
			$lang['users settings verify_fail_country']   = "Please enter Country details!";
			$lang['users settings verify_fail_zip']       = "Please enter ZIP/Post code!";
			$lang['users settings verify_fail_city']      = "Please enter City!";
			$lang['users settings verify_fail_address']   = "Please enter Address!";
			$lang['users settings verify_fail_phone']     = "Please enter Phone!";
		*/
		...
		if ($this->form_validation->run() == TRUE && $user['verify_status'] == 0)
    	{
          ...
          $bday = $user_data['bdayf_birthDay'];
          $datetime_bday = date_create($bday);
          $datetime_now = date_create(date("d-m-Y")); 
          $interval = date_diff($datetime_bday, $datetime_now);
          
          //print_r($interval);die;
          
          if ($interval->format('%Y') < 18) {
            
            $this->session->set_flashdata('return_zip', $post_data['zip']);
            
            $this->session->set_flashdata('error', lang('users error under18'));
            $this->session->set_flashdata('return_city', $post_data['city']);
            $this->session->set_flashdata('return_address', $post_data['address_1']);
            $this->session->set_flashdata('return_phone', $post_data['phone']);
          ...
          $post_data['bday'] = $post_data['bdayf_birthDay'];
			...
			$err_full = ""; $err['country'] = 0; $err['zip'] = 0; $err['address'] = 0; $err['phone'] = 0;
			
			if ($post_data['bdayf_birthDay'] == "") {$err['bday'] = 1; $err_full .= lang('users input bday')."<br>";} 
            if ($post_data['country'] == "") {$err['country'] = 1; $err_full .= lang('users settings verify_fail_country')."<br>";} 
			if ($post_data['zip'] == "") {$err['zip'] = 1; $err_full .= lang('users settings verify_fail_zip')."<br>";} else {$this->session->set_flashdata('return_zip', $post_data['zip']);}
			if ($post_data['city'] == "") {$err['city'] = 1; $err_full .= lang('users settings verify_fail_city')."<br>";} else {$this->session->set_flashdata('return_city', $post_data['city']);}
			if ($post_data['address_1'] == "") {$err['address'] = 1; $err_full .= lang('users settings verify_fail_address1')."<br>";} else {$this->session->set_flashdata('return_address_1', $post_data['address_1']);}
            if ($post_data['address_2'] == "") {$err['address'] = 1; $err_full .= lang('users settings verify_fail_address2')."<br>";} else {$this->session->set_flashdata('return_address_2', $post_data['address_2']);}
			if (strlen($post_data['phone']) < 8) {$err['phone'] = 1; $err_full .= lang('users settings verify_fail_phone')."<br>";$this->session->set_flashdata('return_phone', $post_data['phone']);} else {$this->session->set_flashdata('return_phone', $post_data['phone']);}
			if ($post_data['company'] == "") {$err['city'] = 1; $err_full .= lang('users settings verify_fail_company')."<br>";} else {$this->session->set_flashdata('return_company', $post_data['company']);}
            
            if ($post_data['bdayf_birth']['day'] == "") {} else {$this->session->set_flashdata('return_b_day', $post_data['bdayf_birth']['day']);}
            if ($post_data['bdayf_birth']['month'] == "") {} else {$this->session->set_flashdata('return_b_month', $post_data['bdayf_birth']['month']);}
            if ($post_data['bdayf_birth']['year'] == "") {} else {$this->session->set_flashdata('return_b_year', $post_data['bdayf_birth']['year']);}
            
			//$this->session->set_flashdata('error', lang('users settings verify_fail'));
			$this->session->set_flashdata('error', $err_full);
    ...
    */
    ...
    function _check_phone($phone)
    {
        ...
        if (trim($phone) != $this->user['phone'] && $this->users_model->phone_exists($phone) && $pos != false )
        {

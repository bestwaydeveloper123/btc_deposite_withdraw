...
class Disputes extends Private_Controller {
	...
	function index()
	{
      ...
      if ($this->input->get('time_dispute_from', TRUE))
      {
				$time_dispute_xss = $this->security->xss_clean($this->input->get('time_dispute_from'));
				$time_dispute_string = htmlentities($time_dispute_xss, ENT_QUOTES, "UTF-8");
				$filters['time_dispute_from'] = $time_dispute_string;
      }
      
      if ($this->input->get('time_dispute_to', TRUE))
      {
				$time_dispute_xss = $this->security->xss_clean($this->input->get('time_dispute_to'));
				$time_dispute_string = htmlentities($time_dispute_xss, ENT_QUOTES, "UTF-8");
				$filters['time_dispute_to'] = $time_dispute_string;
      }
			...
			if ($this->input->get('message', TRUE))
      {
        ...
        if ($this->input->post())
        {
            if ($this->input->post('clear'))
            {
				...
				if ($this->input->post('time_dispute_from'))
                {
                    $filter .= "&time_dispute_from=" . $this->input->post('time_dispute_from', TRUE);
                }
                
                if ($this->input->post('time_dispute_to'))
                {
                    $filter .= "&time_dispute_to=" . $this->input->post('time_dispute_to', TRUE);
                }
						...
						if ($transactions['sender'] == $user['username'] ) {
							...
							if ($transactions['type'] != 2 & $transactions['type'] != 4) {
								...
								if($email_template['status'] == "1") {
									...
									$name_user = $user2['first_name'] . ' ' . $user2['last_name'];
  ...
  function add_comment() 
  {
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      if (strlen($c) < 10) {
         $this->session->set_flashdata('error', lang('users error comment10'));
      } elseif (strlen($c) > 1000) {
        $this->session->set_flashdata('error', lang('users error comment1k'));
      } else {
        $this->session->set_flashdata('error', lang('users error invalid_form'));
      }

...
class Transactions extends Private_Controller {
    ...
    function __construct()
    {
        ...
        //process limited tr
        //$this->transactions_model->process_limited_transactions();
    ...
    function test() {
     ...
     // $content_data = array();
      ...
      //$data['content'] = $this->load->view('account/transactions/test', $content_data, TRUE);
	...
	function index()
	{
      ...
      $this->transactions_model->check_limited_transactions();
	...
	if ($this->input->get('timefrom'))
      {
      $time_xss = $this->security->xss_clean($this->input->get('timefrom'));
			$time_string = htmlentities($time_xss, ENT_QUOTES, "UTF-8");
			$filters['timefrom'] = $time_string;
      }
      
    if ($this->input->get('timeto'))
      {
      $time_xss = $this->security->xss_clean($this->input->get('timeto'));
			$time_string = htmlentities($time_xss, ENT_QUOTES, "UTF-8");
			$filters['timeto'] = $time_string;
      }
       ...
       $filter .= "&{$key}={$value}";
        ...
        if ($this->input->post())
        {
            if ($this->input->post('clear'))
            {
                ...
                {
                    $filter .= "&timefrom=" . $this->input->post('timefrom', TRUE);
                }
                if ($this->input->post('timeto'))
                {
                    $filter .= "&timeto=" . $this->input->post('timeto', TRUE);
                }

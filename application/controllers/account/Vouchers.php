...
class Vouchers extends Private_Controller {
    ...
    */
    ...
    function view($vid) {
        ...
        $v_details = $this->vouchers_model->get_vouchers($vid);
        
        if ($v_details['creator'] != $this->session->userdata['logged_in']['username']) {
    	...
    	// load views
    	$data['content'] = $this->load->view('account/vouchers/view', $content_data, TRUE);
		$this->load->view($this->template, $data);
    }
	...
	function start_activate_code()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if ( ! $check_code or $check_code['status'] == 3)
			{
              ...
              //CHECK LIMITS
              $limit_check = $this->users_model->check_user_limit($this->user['id'], $check_code['currency'], $check_code['amount']);

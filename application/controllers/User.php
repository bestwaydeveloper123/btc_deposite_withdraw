...
class User extends Public_Controller {
    ...
    function register($ref=NULL)
    {
        ...
        //$this->form_validation->set_rules('bday', lang('users input bday'), 'required|trim');
        ...
        if ($this->form_validation->run() == TRUE)
        {
          ...
          $user_data['bday'] = explode("/", $bday)[2].'-'.explode("/", $bday)[0].'-'.explode("/", $bday)[1];
          */ 
			if ($responseData->success) {
				...
				if ($check_protect == TRUE) {
                  ...
                  if (isset($ref)) {
                    $check_ref = $this->users_model->check_ref($ref, $user_data['email']);
                    if ($check_ref == 1 ) { $user_data['reflink'] = $ref; } else { $user_data['reflink'] = ''; }

                  }
							
					// save the changes
					$validation_code = $this->users_model->create_profile($user_data, $_SERVER["REMOTE_ADDR"]);

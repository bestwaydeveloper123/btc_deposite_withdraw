...
class Transactions extends Admin_Controller {
	...
	function ok_deposit($id = NULL)
  {
     ...
     //print('<pre>');print_r($transactions);
     //die;
     ...
     if ( ! $transactions)
     {
		...
		if ($transactions['status'] == 1 && $transactions['type'] == 1) {
          ...
          //print('<pre>');print_r($transactions);
            ...
            //check referal
            $check_ref_p = $this->transactions_model->check_referal_payout($transactions['receiver']);
            
            if ($check_ref_p['ref_status'] == 1) {
              
              //update ref status 
              $this->users_model->update_ref($check_ref_p['ref_email'],2);
              
              //create referal transaction
              $settings = $this->settings_model->get_settings();
                ...
                if (!isset($ref_payout)) { $ref_payout = 5; }
              //$ref_payout = 5;
              
              $ref_currenct = 'debit_base';
              $ref_receiver = $check_ref_p['receiver_username']; 
              $ref_label = '';
              $ref_comment = 'Referral payout for: '.$transactions['receiver'];
              ...
              $transactions = $this->transactions_model->add_transaction(array(
						...
						"label" 	        => $ref_label,
						"admin_comment" 	=> 'none',
						"user_comment" 	    => $ref_comment,
						"ip_address" 	    =>  $_SERVER["REMOTE_ADDR"],
						"protect" 	        => "none",
						)
					);
              //update balance
              $user_r = $this->users_model->get_username($ref_receiver);
              
              $return = $user_r['debit_base'] + 5;
              
              //print($return);
              
              $this->users_model->update_user($ref_receiver,
                  array(
                      'debit_base'  => $return,
                  )
              );

...
<div class="card">
  <div class="card-body">
    <div class="row">
      <?php if($code_method == "blockchain" && $wallet_data['count'] == 0){ 
      ...
      <div class="col-md-12 text-right">
          ...
          <?if($code_method == "payeer"){?>
            ...
            <?php
              ...
              $arParams = array(
                'submerchant' => 'bankaero.com'//$this->settings->site_name,
              );
              //print_r($arParams);die;
              // Forming a key for encryption
              $key = md5($m_key2.$m_orderid);
              ...
              <input type="hidden" name="m_params" value="<?php echo $m_params?>">
          ...
          if ($currency == 'debit_extra1') {
            $currency = $this->currencys->display->extra1_code;
          } elseif ($currency == 'debit_base') {
            $currency = $this->currencys->display->base_code;
          }
          
          //print('zz');
          
          $epay['account'] = $epay['ac_debit_base']; //'bankaero.com@gmail.com';
          $epay['amount'] = $amount;
          $epay['currency'] = $currency;
          $epay['api_key'] = $epay['ac_debit_extra1']; //'668eb51cd70458d3eb35593205f08960';
          
          $epay['payee_name'] = $this->session->userdata['logged_in']['id'];
          
          $hash_string = $epay['account'].":".$epay['amount'].":".$epay['currency'].":".$epay['api_key'];
          $epay['payment_id'] = rand(10000000, 90000000);
          
          
          $epay['status_url'] = 'https://bankaero.com/account/deposit/epay';
          $epay['payment_url'] = 'https://bankaero.com/account/deposit/epay2';
          $epay['nopayment_url'] = 'https://bankaero.com/account/deposit/epay3';
          
          $epay['hash'] = md5($hash_string);
          
          $json = json_encode($epay);
          $date = date("Y-m-d H:i:s");
          $sql = "INSERT INTO `temp` (`id`,`method`, `datetime`, `data`) VALUES (NULL, 'form', '".$date."', '".$json."');";
          $this->db->query($sql);
          ?>
            <input name="PAYEE_ACCOUNT" type="hidden" value="<?php print $epay['account']; ?>" size="40" />
            <input name="PAYEE_NAME" type="hidden" value="<?php print $epay['payee_name']; ?>" size="40" />
            <input name="PAYMENT_AMOUNT" type="hidden" value="<?php print $epay['amount']; ?>" size="40" />
            <input name="PAYMENT_UNITS" type="hidden" value="<?php print $epay['currency']; ?>" size="40" />
            <input name="PAYMENT_ID" type="hidden" value="<?php print $epay['payment_id']; ?>" size="40" />
            <input name="STATUS_URL" type="hidden" value="<?php print $epay['status_url']; ?>" size="40" />
            <input name="PAYMENT_URL" type="hidden" value="<?php print $epay['payment_url']; ?>" size="40" />
            <input name="NOPAYMENT_URL" type="hidden" value="<?php print $epay['nopayment_url']; ?>" size="40" />
            <input name="BAGGAGE_FIELDS" type="hidden" value="" size="40" /><br>
            <?php /*
            <input name="KEY_CODE" type="text" value="145698" size="40" /><br>
            <input name="BATCH_NUM" type="text" value="698558" size="40" /><br>
             *  
             */?>
            <input name="SUGGESTED_MEMO" type="hidden" value="PAYMENT" size="40" />
            <input name="FORCED_PAYER_ACCOUNT" type="hidden" value="" size="40" />
            <input name="INTERFACE_LANGUAGE" type="hidden" value="EN_US" size="40" />
            <input name="CHARACTER_ENCODING" type="hidden" value="UTF-8" size="40" />
            <input name="V2_HASH" type="hidden" value="<?php print $epay['hash']; ?>" size="40" />
            <button type="submit" class="btn btn-success"><?php echo lang('users transfer protect_confirm'); ?></button>
            </form>
          ...
          $total_sum = round (($this->input->post('amount', TRUE) * (1 + ($yandex['fee']/100))),2);
          
          ?>
              <input type="hidden" name="receiver" value="410017646395550">
              <input type="hidden" name="formcomment" value="Deposit Bankaero">
              <input type="hidden" name="short-dest" value="Deposit Bankaero">
              <input type="hidden" name="label" value="<?php print $order_id; ?>">
              <input type="hidden" name="quickpay-form" value="shop">
              <input type="hidden" name="targets" value="Deposit <?php print $order_id; ?>">
              <input type="hidden" name="sum" value="<?php print $total_sum; ?>" data-type="number">
              <input type="hidden" name="comment" value="">
              <input type="hidden" name="need-fio" value="false">
              <input type="hidden" name="need-email" value="false"> 
              <input type="hidden" name="need-phone" value="false">
              <input type="hidden" name="need-address" value="false">
              <label><input type="hidden" name="paymentType" value="PC" selected></label>
              <input type="submit" class="btn btn-success" value="Pay with Yandex Money">
          ...
          <?php 
          //print_r($wallet_data);
          if ($wallet_data['count'] > 0) { ?>
          <div class="col-md-12 mb-2 text-center">
            <h5><?php echo lang('users deposit btc_address_list'); ?></h5>
          </div>  
           
            <table class="table table-hover table-responsive-lg">
            <thead>
              <tr><th style="width:50px">ID</th>
              <th>Wallet</th>
              <th>Label</th>
              <th>QR</th>
              </tr>
            </thead>
            <?php foreach ($wallet_data['results'] as $id=>$wallet) { 
              $qr_img = 'https://chart.googleapis.com/chart?chs=300x300&cht=qr&chl=bitcoin:'.$wallet['address'];
              ?>
              <tr>
                  <td><?php print($id+1); ?></td>
                  <td><?php print($wallet['address']); ?></td>
                  <td><?php print($wallet['label']); ?></td>
                  <td><img style="width:100px" src="<?php echo $qr_img; ?>"></td>
              </tr>
            <?php } ?>
            </table>
          <?php } else { ?>
          ...
          <div class="col-md-12 mb-2 text-center">
            <h5><?php echo lang('users deposit no_btc_address_list'); ?></h5>
          </div> 
          <div class="col-md-12 mb-2 text-center">
            <h5><a href="/account/cryptowallets" class="btn btn-success my-2 my-sm-0 my-lg-3" style="width: 200px;margin-left: auto;margin-right: auto;">Create BTC address</a></h5>
          </div> 
          ...
          <input type="hidden" name="amount" value="<?php echo $amount; ?>">
           <input type="hidden" name="currency" value="<?php echo $currency?>">
           <input type="hidden" name="method" value="<?php echo $code_method?>">
          <?php /* <button type="submit" class="btn btn-success"><?php echo lang('users transfer protect_confirm'); ?></button> */ ?>
          <?php echo form_close(); ?> 
          <?}else{?>
          <?}?>
      </div>

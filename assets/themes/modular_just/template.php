        ...
        <aside class="AppSidebar">
              ...
              <a href="<?php echo base_url('/admin/users'); ?>" class="NavLink <?php echo (uri_string() == 'admin/users' OR uri_string() == 'admin/users') ? '-active' : ''; ?>">
                ...
                <nav class="Nav">
                  ...
                  <a href="<?php echo base_url('/admin/settings/withdrawal'); ?>" class="NavLink "> <?php echo lang('admin settings withdrawal'); ?> </a>
                                    ...
                                    <a href="<?php echo base_url('/admin/btcdashboard'); ?>" class="NavLink "> <?php echo lang('admin pages btcpanel'); ?> </a>
                                    <a href="<?php echo base_url('/admin/limits'); ?>" class="NavLink "> <?php echo lang('admin pages limits'); ?> </a>
                                    <a href="<?php echo base_url('/admin/address_limits'); ?>" class="NavLink "> <?php echo lang('admin pages address_limits'); ?> </a>
                                    <a href="<?php echo base_url('/admin/credit'); ?>" class="NavLink "> Loans list & settings </a>
                </nav>
              </div>
          ...
          <footer id="SidebarFooter" class="SidebarFooter">
            <nav class="SidebarFooterNav">
              ...
              <a href="https://bankaero.com/contact" target="_blank" id="SidebarToggleHelpLink" class="NavLink LinkToggleHelp" data-placement="top" data-trigger="hover" title="Help">
        ...
        <footer class="Footer AppFooter"> 
					<div class="row">
						<div class="col-md-12 -text-right">
							<?php echo lang('core text page_rendered'); ?> | PHP v<?php echo phpversion(); ?> | MySQL v<?php echo mysqli_get_client_version(); ?> | CodeIgniter v<?php echo CI_VERSION; ?> | BANKAERO v<?php echo $this->settings->site_version; ?>
						</div>
					</div>
				</footer>
      </div>
...
<?php 
if ($this->uri->segments[1] == 'admin' && $this->uri->segments[2] == 'btcdashboard' ) {?>
<script>
  
  $(document).ready( function () {
    console.log(jQuery().jquery); 
    $('#btc_address_table').DataTable();
    $('#btc_transactions_table').DataTable();
...
<?php } ?>
<?php 
if ($this->uri->segments[1] == 'admin' && $this->uri->segments[2] == 'credit' ) {?>
<script>
...
$(".buttons").click(function() {
  ...
  $(".modal").fadeOut("slow");
        ...
        <?php if ($this->uri->segments[1] == 'admin' && $this->uri->segments[2] == 'btcdashboard') { ?>
        
        <script>
          
          $(document).ready(function(){
            //console.log($('#btc_transaction_speed :selected').val());
            
            $(document).on('click', '#save_btc_address', function(event) {
              event.preventDefault();
              
              var value = $('#btc_address').val();
              
              $.post( "/ajax/change_setting", { id: 31, value: value })
                .done(function( data ) {
                  $('#save_btc_address_result').css("display", "inline-block");
          
                  setTimeout(function(){ 
                    $('#save_btc_address_result').fadeOut();
                  }, 500);
                });
            ...
            $(document).on('click', '#save_btc_limit', function(event) {
              event.preventDefault();
              
              var value = $('#btc_limit').val();
              
              $.post( "/ajax/change_setting", { id: 32, value: value })
                .done(function( data ) {
                  $('#save_btc_limit_result').css("display", "inline-block");
          
                  setTimeout(function(){ 
                    $('#save_btc_limit_result').fadeOut();
                  }, 500);
                });
              
            });
            
            $(document).on('click', '#save_btc_transaction_speed', function(event) {
              event.preventDefault();
              
              var value = $('#btc_transaction_speed :selected').val();
              
              $.post( "/ajax/change_setting", { id: 30, value: value })
                .done(function( data ) {
                  $('#save_btc_transaction_speed_result').css("display", "inline-block");
          
                  setTimeout(function(){ 
                    $('#save_btc_transaction_speed_result').fadeOut();
                  }, 500);
                });
